...
# ✅ Автоматический переход на следующий день при отсутствии слотов
# ✅ Только будущие свободные слоты
# ✅ Совместимость с aiogram 3.13+ и Render
import os
import json
import logging
...
def get_working_slots(date_iso: str) -> list[str]:
    ...
    # Вечер: до 20:00 (пн–пт) → 19:30; сб → 17:30
...
async def fetch_free_slots(date_iso: str) -> list[str]:
    """Возвращает ТОЛЬКО свободные слоты для указанной даты."""
...
# 🔹 🔹 🔹 КЛЮЧЕВОЙ ХЭНДЛЕР: day — с автоматическим переходом на следующий день
...
async def day(cb: CallbackQuery, state: FSMContext):
    ...
    # 🔹 Фильтрация: только будущее время (если дата — сегодня)
    now = datetime.now(TZ).replace(tzinfo=None)
    ...
    # 🔹 🔹 🔹 Если нет свободных слотов — ищем ближайший день
    if not slots:
        logger.info(f"🕗 Нет свободных слотов на {date_iso}. Ищу следующий день...")
        next_date = datetime.fromisoformat(date_iso)
        for _ in range(7):
            next_date += timedelta(days=1)
            if next_date.weekday() == 6:  # воскресенье
                continue
            next_iso = next_date.strftime("%Y-%m-%d")
            next_slots = await fetch_free_slots(next_iso)
            if next_slots:
                # Переключаемся на найденный день
                ...
                # Рекурсивный вызов с новой датой
